...
实际的指令位于第 5 行，也即 ``lu12i.w $r1, 100`` 。 LU12I.W 指令格式为 ``lu12i.w rd, si20`` ，它将 20 比特立即数 ``si20`` 最低位连接上 12 比特 0，然后符号扩展后写入通用寄存器 ``rd`` 中。第 4 行我们声明了一个符号 ``main`` ，该符号指向紧跟在符号后面的内容——也就是位于第 5 行的指令，因此符号 ``main`` 的地址即为第 5 行的指令所在的地址。第 3 行我们告知编译器 ``main`` 是一个全局符号，因此可以被其他目标文件使用。第 2 行表明我们希望将第 2 行后面的内容全部放到一个名为 ``.text.entry`` 的段中。一般情况下，所有的代码都被放到一个名为 ``.text`` 的代码段中，这里我们将其命名为 ``.text.entry`` 从而区别于其他 ``.text`` 的目的在于我们想要确保该段被放置在相比任何其他代码段更低的地址上。这样，作为内核的入口点，这段指令才能被最先执行。

.. hint:: 请思考：执行这条指令后，``$r1`` 寄存器中的值是多少？
...
由于链接器默认的内存布局并不能符合我们的要求，为了实现与 Qemu 正确对接，我们可以通过 **链接脚本** (Linker Script) 调整链接器的行为，使得最终生成的可执行文件的内存布局符合Qemu的预期，即内核第一条指令的地址应该位于 `0x9000000000100000` （实际被加载到 `0x100000` ）。我们修改 Cargo 的配置文件来使用我们自己的链接脚本 ``os/src/linker.ld`` 而非使用默认的内存布局：
...
.. code-block:: 
    :linenos:
    :emphasize-lines: 2, 3, 4, 13

    # os/src/linker.ld
    OUTPUT_ARCH(loongarch)
    ENTRY(main)
    BASE_ADDRESS = 0x9000000000100000;
...
第 2 行我们设置了目标平台为 loongarch ；第 2 行我们设置了整个程序的入口点为之前定义的全局符号 ``main``；

第 5 行定义了一个常量 ``BASE_ADDRESS`` 为 ``0x9000000000100000`` ，也就是我们之前提到内核的初始化代码被放置的地址；

从第 6 行开始体现了链接过程中对输入的目标文件的段的合并。其中 ``.`` 表示当前地址，也就是链接器会从它指向的位置开始往下放置从输入的目标文件中收集来的段。我们可以对 ``.`` 进行赋值来调整接下来的段放在哪里，也可以创建一些全局符号赋值为 ``.`` 从而记录这一时刻的位置。我们还能够看到这样的格式：
...
第 13 行我们将包含内核第一条指令的 ``.text.entry`` 段放在最终的 ``.text`` 段的最开头，同时注意到在最终内存布局中代码段 ``.text`` 又是先于任何其他段的。因为所有的段都从 ``BASE_ADDRESS`` 也即 ``0x9000000000100000`` 开始放置，这就能够保证内核的第一条指令正好放在 ``0x9000000000100000`` 从而能够正确对接到 Qemu 上。
...
.. code-block:: console
    ...
    target/loongarch64-unknown-none/release/os: ELF 64-bit LSB executable, *unknown arch 0x102* version 1 (SYSV), statically linked, for GNU/Linux 5.15.0, with debug_info, not stripped

我们以 ``release`` 模式生成了内核可执行文件，它的位置在 ``os/target/loongarch64-unknown-none/release/os`` 。接着我们通过 ``file`` 工具查看它的属性，可以看到目前 ``file`` 工具还不能识别其架构，但可以得到其编号为 ``0x102``，参考 :ref:`前面 <rust-readobj-0x102>` ``rust-readobj`` 结果中的 ``Machine: EM_LOONGARCH (0x102)`` ，我们可以知道它就是指 LoongArch 架构。 ``file`` 还告诉我们该文件是静态链接得到的。

.. note::

    **绝对地址与相对地址**

    在对编译器进行某些设置的情况下，在访问变量或函数时，可以通过它们所在地址与当前某个寄存器（如 PC）的相对地址而非它们位于的绝对地址来访问这些变量或函数。比如，在一个起始地址（即上面提到的 ``BASE_ADDRESS`` ）固定为 ``0x9000000000100000`` 的内存布局中，某个函数入口位于 ``0x0x9000000000101111`` 处，那么我们可以使用其绝对地址 ``0x9000000000101111`` 来访问它。但是，如果一条位于 ``0x9000000000100111`` 指令会调用该函数，那么这条指令也不一定要用到绝对地址 ``0x9000000000101111`` ，而是用函数入口地址相对于当前指令地址 ``0x9000000000100111`` 的相对地址 ``0x1000`` （计算方式为函数入口地址与当前指令地址之差值）来找到并调用该函数。
...
.. code-block:: console
    ...
    $ qemu-system-loongarch64 \
        -vga none -nographic \
        -bios ../bios/loongarch_bios_0310_debug.bin \
...
可以看到，正如我们在上一节提到的那样，bios 的起始地址为 ``0x1c000000`` 。我们可以在我们预先设定的内核起始地址处设置一个断点：
...
我们在内核的入口点，也即地址 ``0x100000`` 处打一个断点。需要注意，当需要在一个特定的地址打断点时，需要在地址前面加上 ``*`` 。接下来通过 ``c`` 命令（Continue 的缩写）让 Qemu 向下运行直到遇到一个断点。此时观察 QEMU 可以发现其向终端输出了大量内容，这展示了 BIOS 所做的工作。一段时间后可以看到，我们成功停在了 ``0x100000`` 处。此时，观察 QEMU 可以发现其最后一行输出刚好是 ``entry kernel ...`` ，这表明它将要将控制权转让给内核。随后，可以检查内核第一条指令是否被正确执行：

.. code-block:: console

    $ (gdb) x/5i $pc
    => 0x100000:	lu12i.w     	$ra, 1(0x1)
    0x100004:	0x00000000
    0x100008:	0x00000000
    0x10000c:	0x00000000
    0x100010:	0x00000000
    (gdb) si
    0x0000000000100004 in ?? ()
    (gdb) p/d $r1
    $1 = 4096
    (gdb) p/x $sp
    $2 = 0x9f7a4cb8

可以看到我们在 ``entry.asm`` 中编写的第一条指令可以在 ``0x100000`` 处找到。这里 ``ra`` 是寄存器 ``r1`` 的别名， ``p/d $r1`` 可以以十进制打印寄存器 ``r1`` 的值。它的结果与你前面思考的答案一致吗？最后，作为下一节的铺垫，我们可以检查此时栈指针 ``sp`` 的值，可以发现它目前是 0x9f7a4cb8 。下一节我们将进一步讨论栈与函数调用，随后将控制权转交给 Rust 代码。
